...
#include <signal.h>
#include "Practical.h"
char *server = "", *echoString = "", *servPort = "", *servHead = "";
int count = 0, start = 0;
char *message[80];
double time_used = 0;
char buffer[BUFSIZE];
...
int main(int argc, char *argv[]) {
 // signal(SIGINT, Handler);
  int start = clock();
  int i = 0;
  if(argc < 8 || argc % 2 != 0) DieWithUserMessage("Parameter(s)", 
      "<Server Address><Server Port><Host Header><Message(s)>");
  for(i = 0; i < (argc-1); i+=2){
        if(strcmp(argv[i] , "-s") == 0) server = argv[i+1];
        else if(strcmp(argv[i] , "-p") == 0) servPort = argv[i+1];
        else if(strcmp(argv[i] , "-h") == 0) servHead = argv[i+1];
        else if(strstr(argv[i] , "-a") != NULL) message[i+1] = argv[i+1];   
    }  
  ...
  for(int i = 0; i < 9; i+=2){
        if(strcmp(argv[i] , "-s") == 0) server = argv[i+1];
        else if(strstr(argv[i] , "-a") == 0) echoString = argv[i+1];
        else if(strcmp(argv[i] , "-p") == 0) servPort = argv[i+1];
        else if(strcmp(argv[i] , "-h") == 0) servHead = argv[i+1];
    }
  }*/
