...
void flt_bin (int part, int len){   
  for(int i = len; i>=0;i--){
	if((part>>i)&1) putchar('1');
        else putchar('0');
       	if(i == 6) putchar('.');
 	if(i == 2) putchar(' ');
	}
}
//prints out steps of floating point addition process
...
float val2, float answer, float answer2){
   //these four variables store original values for final print statements.
   int origexp = exp;
   int origexp2 = exp2;
   int origsign = sign;
   int origsign2 = sign2;
   cout << endl << "addition of the values:" << endl;
   if (exp2 > exp) {//swapping values if the first has a smaller exponent.
        swap(exp, exp2);
        swap(frac1, frac2);
        swap(sign, sign2);
    	cout << endl << "operands are swapped" << endl;
	cout << "internal rep of first value:   "; flt_bin(frac1, 6);
	 
	if(exp >= 0) cout << " x 2^(+" << exp <<")" << endl;
	else cout << " x 2^(" << exp <<")" << endl;
	  
	cout << "internal rep of second value:  "; flt_bin(frac2, 6);

	if(exp2 >= 0) cout << " x 2^(+" << exp2 <<")" << endl;
	else cout << " x 2^(" << exp2 <<")" << endl; 
	}
  else cout << endl;
  while(exp2 != exp){//shifting second value until both exponents are equal.
  ...
  //negates operands if they are different signs (take two's complement)
  if(sign && !sign2){
	frac1 = ~(frac1-1);
  ...
  //prints out sum of first and second values.
  cout <<"internal rep of sum:          "; flt_bin(sum, 7);
  if(expsum >= 0) cout << " x 2^(+" << expsum <<")" << endl;
  else cout << " x 2^(" << expsum <<")" << endl;
  //if 1.fraction becomes 10.fraction
  if(sum > 127) {
	sum = sum >> 1;
	expsum += 1;
  ...
  //if 1.fraction becomes 0.fraction
  if(sum < 64){
	cout << "sum normalized" << endl;
	sum = sum << 1;
	expsum -= 1;
  ...
  //chooses what to print if exp is pos/neg and if the sign is pos/neg
  if(origexp >= 0 && !origsign){
        cout << endl << "               +" << val << " x 2^(+";
	cout << origexp <<") = " << answer << endl;
        }
  else if(origexp >= 0 && origsign){
        cout << endl << "               -" << val << " x 2^(+";
  ...
  //chooses what to print if exp2 is pos/neg and if the sign2 is pos/neg
  if(origexp2 >= 0 && !origsign2){
        cout << "      added to +" << val2 << " x 2^(+"; 
        cout << origexp2 <<") = " << answer2 << endl;
        }
  else if(origexp2 >= 0 && origsign2){
        cout << "      added to -" << val2 << " x 2^(+"; 
  ...
  //chooses what to print if expsum is pos/neg and if signsum is pos/neg
  if(expsum >= 0 && !signsum){
...
int main(void){
  ...
  cin >> num1 >> num2;  
  //this gives a positive/negative sign to num1 and num2.
  sign1 = (num1 >> 7) & 1;
  sign2 = (num2 >> 7) & 1;
  
  unsigned int frac1 = 64 + ((num1 & 15) << 2);
  unsigned int frac2 = 64 + ((num2 & 15) << 2);
  //creates exponents for num1 and num2 (also applies bias (-4))
  int exp = ((num1 >> 4) & 7)-4;
  int exp2 = ((num2 >> 4) & 7)-4;

  float val1 = 1 + shifter(frac1);
  float val2 = 1 + shifter(frac2);
  //these ensure that all values have 4 decimal places.
  cout << fixed;
  cout << setprecision(4);  
  ...
  cout << endl << "encoding of first value:      "; prt_bin(num1, 8);
  cout << endl;
  
  cout << "encoding of second value:     "; prt_bin(num2, 8);
  cout << endl;

  cout << endl << "internal rep of first value:   "; flt_bin(frac1, 6);
  if(exp >= 0) cout << " x 2^(+" << exp <<")" << endl;
  else cout << " x 2^(" << exp <<")" << endl;
  ...
  cout << "hidden bit---------------------^ ffff gr" << endl;
  cout << "4-bit fraction-------------------^^^^" << endl;
  //calculates sum of first and second fraction values.
