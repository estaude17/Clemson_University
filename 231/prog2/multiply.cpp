...
void steps(int mq, int mdr, int acc, int c){
  ...
  for(int i = 1; i < 9; i++){//this causes the process to loop for 8 steps.
 	c = 0; //ensures that c is restored to zero after every shift.
	...
	if((mq % 2) == 1){//if the multiplier gives a remainder of 1 (is odd)
		...
		acc = acc + mdr;
		if(acc > 255) c = 1;//this is for potential overflow.
		else c = 0;
                }
	...
	is 2^7 power, or 128.*/
	acc = acc>>1;
 	c = 0;//this ensures that c is zero after the accumulator shifts.
	cout << "          " << c << " ";
	prt_bin(acc, 8);
	cout << " ";
	prt_bin(mq,8);          
