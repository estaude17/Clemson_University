...
#include <sys/types.h>
#include <time.h>
#define CACHE_MIN (512)	/* smallest cache. Change it for your experiments */
#define CACHE_MAX (512*512*16) 	/* largest cache. Change it for your experiments */
...
void main(){
	...
	for (csize = CACHE_MIN; csize <= CACHE_MAX; csize = csize*2){
		for (stride  = 1; stride <= csize/2; stride = stride*2){
			...
			printf("Size : %7d Stride : %7d read+write: %14.1f ns\n", 
