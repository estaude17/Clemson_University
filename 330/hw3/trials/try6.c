//     *********************************
//     *    Matrix Multiply Project    *
//     *                               *
//     *********************************
//     ** MAIN PROGRAM  **
//     *************************************************
//     ** Any changes you make to this code must      **
//     ** maintain the correctness of the matrix      **
//     ** multiply computed by the original version.
//     **					      **
//     ** An implementation with incorrect results for**
//     ** matrix C earns zero point for this project. **
//     ** 
//     ** To print matrix compile with -DPRINT_MATRIX **
//     ** e.g., g++ -DPRINT_MATRIX                    **
//     **                                             **
//     ** A sample Makefile is provided.
//     ** You may assume m = n = k for your matrices  **
//     *************************************************
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <sys/time.h>
#include <math.h>
double **dmatrix(int nrl,int nrh,int ncl,int nch);
void nerror(char *error_text);
#define fmin(a,b)            (((a) < (b)) ? (a) : (b))
int main(int argc, char** argv)  {
 
      struct timeval start, end;
   
      int l,m,n,k;
      int i,j;
      double **A, **B, **C;
      double **temp;
     //  ****************************************************
     //  * The following allows matrix parameters to be     *
     //  * entered on the command line to take advantage    *
     //  * of dynamically allocated memory.  You may modify *
     //  * or remove it as you wish.                        *
     //  ****************************************************
     if (argc != 4) {
       nerror("Usage:  <executable> <m-value> <n-value> <k-value>");
     }
      m = atoi(argv[1]);
      n = atoi(argv[2]);
      k = atoi(argv[3]);
      // *********************************************************
      // * Call the dmatrix() subroutine to dynamically allocate *
      // * storage for the matrix sizes specified by m, n, and k *  
      // *********************************************************
      A=dmatrix(0,m-1,0,k-1);
      B=dmatrix(0,k-1,0,n-1);
      C=dmatrix(0,m-1,0,n-1);
   
      //maybe we should do this for our transpose array too
      temp = dmatrix(0,k-1,0,n-1);
      // *********************************************************
      //  * Initialize matrix elements so compiler does not      *
      //  * optimize out                                         *
      // *********************************************************
      //initialize matrix A; note A is symmetric
      //currently initializes down the columns starting from the left
      for(j=0;j<k;j++) {
        for(i=0;i<m;i++) {
          A[i][j] = i+j+4.0;
        }
      }
      //initialize matrix B; note B is symmetric
      for(j=0;j<n;j++) {
        for(i=0;i<k;i++) {
          B[i][j] = i+j+5.0;
        }
      }
      //initialize product matrix to 0
      for(j=0;j<n;j++) {
        for(i=0;i<m;i++) {
          C[i][j] = 0.0;
        }
      }
   
      //initialize our temporary transpose array
       for(j = 0; j < n; j++){
         for(i = 0; i < k; i++){
            temp[i][j] = 0.0;
         }
       }
   
   
      // ******************************
      // * Start embedded timing here *
      // ******************************
      gettimeofday(&start, NULL);
   
      // **********************************
      // * Perform simple matrix multiply *
      // **********************************
   
   //I've included older versions of our code in case we want to retest any of them
   
   //interchange (red portion of table)
   /*
      for(j = 0; j < n; j++){
         for(l = 0; l < k; l++){
            for(i = 0; i < m; i++){
               C[j][l] += A[j][i] * B[i][l];
            }
         }
      }
   
   */
   
   //interchange and peeling (orange)
   /*
      j = 0;
      while(j < n){
         int l = 0;
         while(l < k){
            int i = 0;
            while(i < m){
               C[j][l] += A[j][i] * B[i][l];
               i++;
            }
            l++;
         }
         j++;
      }
   */
   /*
   //interchange, peeling, and unrolling (yellow)
      j = 0;
      while(j < n){
         int l = 0;
         while(l < k){
            int i = 0;
            while(i < m){
               C[j][l] += A[j][i] * B[i][l];
               C[j][l] += A[j][i+1] * B[i+1][l];
               C[j][l+1] += A[j][i] * B[i][l+1];
               C[j][l+1] += A[j][i+1] * B[i+1][l+1];
               C[j+1][l] += A[j+1][i] * B[i][l];
               C[j+1][l] += A[j+1][i+1] * B[i+1][l];
               C[j+1][l+1] += A[j+1][i] * B[i][l+1];
               C[j+1][l+1] += A[j+1][i+1] * B[i+1][l+1];
    
               i += 2;
            }
            l += 2;
         }
         j += 2;
      }
   
   */
   //interchange, peeling, unrolling, and transposition (green)
    
    //transpose array B
      j = 0;
      while(j < n){
         int l = 0;
         while(l < k){
            temp[j][l] = B[l][j];
            l++;
         }
         j++;
      }
   
   /*
    //multiply using transpose
      j = 0;
      while(j < n){
         int l = 0;
         while(l < k){
            int i = 0;
            while(i < m){
               C[j][l] += A[j][i] * temp[l][i];
               C[j][l] += A[j][i+1] * temp[l+1][i];
               C[j][l+1] += A[j][i] * temp[l][i+1];
               C[j][l+1] += A[j][i+1] * temp[l+1][i+1];
               C[j+1][l] += A[j+1][i] * temp[l][i];
               C[j+1][l] += A[j+1][i+1] * temp[l+1][i];
               C[j+1][l+1] += A[j+1][i] * temp[l][i+1];
               C[j+1][l+1] += A[j+1][i+1] * temp[l+1][i+1];
    
               i += 2;
            }
            l += 2;
         }
         j += 2;
      }
    */
   
   
       //blocking added. Man, this is ugly.
       int jj, ll, ii, block;
       block =  48;
       
       jj = 0;
       while(jj < n){
         ll = 0;
         while(ll < k){
            ii = 0;
            while(ii < m){
               j = jj;
               while(j < fmin(jj+block, n)){
                  l = ll;
                  while(l < fmin(ll+block, k)){
                     i = ii;
                     while(i < fmin(ii+block, m)){
                        C[j][l] += A[j][i] * temp[l][i];
                        C[j][l] += A[j][i+1] * temp[l+1][i];
                        C[j][l+1] += A[j][i] * temp[l][i+1];
                        C[j][l+1] += A[j][i+1] * temp[l+1][i+1];
                        C[j+1][l] += A[j+1][i] * temp[l][i];
                        C[j+1][l] += A[j+1][i+1] * temp[l+1][i];
                        C[j+1][l+1] += A[j+1][i] * temp[l][i+1];
                        C[j+1][l+1] += A[j+1][i+1] * temp[l+1][i+1];
       
                        i += 2;
                     }
                     l += 2;
                  }
                  j += 2;
               }
               ii += block;
            }
            ll += block;
         }
         jj += block;
       }
       
   
   
   
   
   
   
   
      // ******************************
      // * Stop embedded timing here  *
      // ******************************
      gettimeofday(&end, NULL);
   
      printf("Matrix multiplication time: %ld microseconds.\n", ((end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec)));
   
      // **************************************************
      // * Print out a 10 x 10 matrix for testing only    *
      // * Comment out when timing                        *
      // **************************************************
/*
 #ifdef PRINT_MATRIX
      fprintf(stdout, "Here is the matrix A:\n\n");
      for(i=0;i<m;i++) {
        for(j=0;j<k;j++) {
          fprintf(stdout, "%10.2f ",A[i][j]);
        }
        fprintf(stdout, "\n");
      }
      fprintf(stdout, "Here is the matrix B:\n\n");
      for(i=0;i<k;i++) {
        for(j=0;j<n;j++) {
          fprintf(stdout, "%10.2f",B[i][j]);
        }
        fprintf(stdout, "\n");
      }
      fprintf(stdout, "Here is the matrix C:\n\n");
      for(i=0;i<m;i++) {
        for(j=0;j<n;j++) {
          fprintf(stdout, "%10.2f",C[i][j]);
        }
        fprintf(stdout, "\n");
      }
#endif        
*/
   
}
//     **  END MAIN PROGRAM  **
//     ********************************************************
//     *******    BEGIN SUBROUTINES    ************************
//     ********************************************************
double **dmatrix(int nrl,int nrh,int ncl,int nch)
// Allocates a double matrix with range [nrl..nrh][ncl..nch]
{
  int i;
  double **m;
//    Allocate pointers to rows
  m=(double **) malloc((unsigned)(nrh-nrl+1)*sizeof(double *));
  if (!m) nerror("allocation failure in malloc in dmatrix()");
  m -= nrl;
//    Allocate rows and set pointers to them
  for(i=nrl;i<=nrh;i++) {
    m[i]=(double*) malloc((unsigned) (nch-ncl+1)*sizeof(double));
    if (!m[i]) nerror("allocaion failure in malloc in dmatrix()");
    m[i] -= ncl;
  }
  return m;
}
void nerror(char *error_text)
{
  void exit();
  fprintf(stderr, "Run-time error...\n");
  fprintf(stderr,"%s\n",error_text);
  fprintf(stderr,"Exiting...\n");
  exit(1);
}

