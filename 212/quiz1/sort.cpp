...
int main(void)
{
  ...
  Node *T = NULL;
  Node *L, *R;
  cout << "Inserting 100000 elements in order...except one"<< "\n";
  for (int i = 0; i < size; i++) 
	{
	cin >> integers;
	T = insert_random(T, integers);
	}
  int random = rand();
  split(T, random, &L, &R);//splits Node T apart into Nodes L & R at random value of T;
  
  if(random < L->size)
	insert_random(L, random);
  else
	insert_random(R, random);
  T= join(L, R);
