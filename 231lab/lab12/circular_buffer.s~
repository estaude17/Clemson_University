...
@CPSC 2311-002
@04/07/2016
@Implements a circular character buffer	
	.syntax unified
	.arch armv7-a
	.eabi_attribute 27, 3
	.fpu vfpv3-d16
...
put:
	...
	mov	r4, r0
	ldr	r0, .L12
	ldr 	r1, [r0] //puts count into r1
	cmp	r1, #8 //checks if r1 is max (8)
	beq	putfull

	ldr	r2, .L12+12 //loads address of cbuf
	ldr	r5, .L12+8 //loads address of in
	ldr	r3, [r5] //store out in r3
...
get:
	...
	ldr	r0, .L17
	ldr	r1, [r0] //puts "count" into r1
	cmp	r1, #0  //checks counter to see if empty
	beq	getempty

	ldr	r5, .L17+12 //load address of cbuf
	ldr	r2, .L17+8  //load address of out
	ldr	r3, [r2] //stores out into r3
	
	ldr	r4, [r5, r3]	
	
	add	r3, r3, #1 //increments out
	and	r3, r3, #7
	str	r3, [r2]
...
getempty:
	ldr 	r0, .L17+4  //print if buffer is empty
	bl	printf
	mov	r0, #0
	pop	{r4, pc}
